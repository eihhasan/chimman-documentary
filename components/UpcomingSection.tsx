...
const UpcomingSection: React.FC = () => {
  return (
    <section className="bg-black py-16 md:py-32 px-6 md:px-24" aria-label="Upcoming Journeys">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16">
          <h2 className="text-white text-3xl md:text-6xl font-black uppercase tracking-tighter ">
            Next Destination.
          </h2>
          <span className="text-zinc-600 font-mono text-xs mt-4 md:mt-0" aria-hidden="true">
            [ THE UNKNOWN CALLS ]
          </span>
        </div>
        ...
        <div className="space-y-8">
          {UPCOMING_JOURNEYS.map((trip, i) => (
            <motion.article
              ...
              tabIndex={0}
              aria-label={`Upcoming trip to ${trip.destination} in ${trip.date}`}
            >
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-4">
                  <span className="flex items-center gap-2 text-white font-bold text-xs uppercase bg-zinc-800 px-3 py-1">
                    ...
                    <span className="sr-only">Destination:</span> {trip.destination}
                  </span>
                  <span className="flex items-center gap-2 text-zinc-500 font-bold text-xs uppercase">
                    <Calendar className="w-3 h-3" aria-hidden="true" />
                    <span className="sr-only">Date:</span> {trip.date}
                  </span>
                </div>
              ...
              <div className="h-full flex items-center">
                <div className="w-12 h-12 rounded-full border border-zinc-700 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all" aria-hidden="true">
